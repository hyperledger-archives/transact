...
mod tests {
    ...
    fn create_contexts() {
        ...
        let first_context_id = manager.create_context(&[], &state_id).unwrap();
        assert!(!manager.contexts.is_empty());
        assert!(manager.contexts.get(&first_context_id).is_some());

        let second_context_id = manager.create_context(&[], &state_id).unwrap();
    ...
    fn add_context_event() {
        let (mut manager, state_id) = make_manager(None);
        let context_id = manager.create_context(&[], &state_id).unwrap();
    ...
    fn add_delete_state_change() {
        ...
        let ancestor_context = manager.create_context(&[], &state_id).unwrap();

        assert!(manager
            .set_state(&ancestor_context, KEY2.to_string(), BYTES2.to_vec())
            .is_ok());

        let current_context_id = manager
            .create_context(&[ancestor_context], &state_id)
            .unwrap();
        assert!(manager
