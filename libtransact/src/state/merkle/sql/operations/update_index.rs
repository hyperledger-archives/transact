...
impl<'a> MerkleRadixUpdateIndexOperation for MerkleRadixOperations<'a, PgConnection> {
    fn update_index(
        &self,
        tree_id: i64,
        state_root: &str,
        ...
        self.conn
            .transaction::<_, diesel::result::Error, _>(|| {
                ...
                let state_root = insert_into(merkle_radix_state_root::table)
                    .values(NewMerkleRadixStateRoot {
                        tree_id,
                        state_root,
                        parent_state_root,
                    })
                    .get_result::<MerkleRadixStateRoot>(self.conn)?;
                ...
                for change in changed_addresses.iter() {
                    ...
                    .set(merkle_radix_state_root_leaf_index::to_state_root_id.eq(state_root.id))
                    .execute(self.conn)?;
                }
...
mod tests {
    ...
    /// This test indexes a leaf and its tree nodes into the index as in an initial entry into the
    /// merkle tree.  That is, there was no prior entry at that address. It Verifies that
    /// 1. the new state root hash specified is inserted into the state root table
    /// 2. the leaf-state-root relationship is added to the index, with a null to_state_root_id
    #[cfg(feature = "state-merkle-sql-postgres-tests")]
    #[test]
    fn postgres_update_index_initial_change() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "state-merkle-sql-postgres-tests")]
    #[test]
    fn postgres_update_index_single_change() -> Result<(), Box<dyn std::error::Error>> {
        run_postgres_test(|url| {
            let conn = PgConnection::establish(&url)?;
    ...
    #[cfg(feature = "state-merkle-sql-postgres-tests")]
    #[test]
    fn postgres_update_index_single_change_fork() -> Result<(), Box<dyn std::error::Error>> {
        run_postgres_test(|url| {
            let conn = PgConnection::establish(&url)?;
